"""
Add 4 CRAG (Corrective RAG) nodes to Flow 7 (VBfS8H71yz0ArkWT).
Inserts into the NO branch of "IF: is_job_related":
...
# --- JS code for CRAG: Parse Retry ---
CRAG_PARSE_JS = r"""const body = $input.item.json;
const origNI = $('Normalize Item').item.json.normalized_input;
let jobEvent;
try {
...
LLM_BODY = ("={{ JSON.stringify({ model: 'qwen/qwen3-30b-a3b-instruct-2507', max_tokens: 2048,"
            ...
            " { role: 'user', content: JSON.stringify({ ...($('Normalize Item').item.json.normalized_input),"
            " raw_text: ($('Normalize Item').item.json.normalized_input.raw_text || '') +"
...
new_nodes = [
    {
        ...
        "parameters": {
            "url": "=https://r.jina.ai/{{ $('Normalize Item').item.json.normalized_input.raw_meta.url }}",
...
try:
    with urllib.request.urlopen(r, context=CTX) as resp:
        ...
        crag_nodes = [n["name"] for n in result["nodes"] if n["name"].startswith("CRAG:")]
        print(f"SUCCESS! Total nodes: {len(result['nodes'])}")
        print(f"CRAG nodes: {crag_nodes}")
        # Verify IF NO branch
